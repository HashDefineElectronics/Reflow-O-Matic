...
void HardwareSetup(void)
{
	...
	RCC->CFGR = (RCC->CFGR & ~((uint32_t)RCC_CFGR_SW_HSI)) | ((uint32_t)RCC_CFGR_SW_PLL); // set the system clock to PLL

	while( (RCC->CFGR & RCC_CFGR_SWS) != RCC_CFGR_SWS_1 ); // ensure that the clock have been switched
